...
//import com.chomp.wifistorymachine.ui.chat.Mp3Recorder.AudioStateListenter;
import com.chomp.wifistorymachine.ui.chat.AudioManage.AudioStateListenter;
...
public class AudioRecorderButton extends Button implements AudioStateListenter {
	...
	public AudioRecorderButton(Context context, AttributeSet attrs) {
		...
		String dir =context.getFilesDir().toString();                             // 此处需要判断是否有存储卡(外存)
		mAudioManage = AudioManage.getInstance(dir);
		mAudioManage.setOnAudioStateListenter(this);

		//mp3Recorder = Mp3Recorder.getInstance(dir);
		//mp3Recorder.setOnAudioStateListenter(this);
		...
		setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				...
				mAudioManage.prepareAudio(getContext());
//				try {
//				   mp3Recorder.startRecording();
//				} catch (Exception e) {
//					Log.d("zcw", "Faile to join encode thread");
//				}
				//return true;
				return false;
			}
...
//					}
					...
					if(mListenter!=null){
						...
						//mListenter.onFinish(mTime, mp3Recorder.getCurrentFilePath());
					}
	...
	private void changeState(int state) {
		if (mCurState != state) {
			...
			switch (state) {
				case STATE_NORMAL:
					setBackgroundResource(R.drawable.btn_chat_recorder);
					//setText(R.string.str_recorder_normal);
					break;

				case STATE_RECORDERING:
					setBackgroundResource(R.drawable.btn_chat_recorder2);
				...
				case STATE_WANT_TO_CALCEL:
					setBackgroundResource(R.drawable.btn_chat_recorder3);
...
//					audioDialogManage.updateVoiceLevel(mp3Recorder
//							.getVoiceLevel(7));
					break;
